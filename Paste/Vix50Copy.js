...
const waitForAuthorization = async () => {
  return new Promise((resolve) => {
    const subscription = observable.subscribe((message) => {
      if (message && message.data && message.data.authorize !== undefined) {
        resolve();
        subscription.unsubscribe(); // Cancela a subscrição após obter a autorização
      }
    });
  });
};

const subscribeToAuthorizationAndGetBalance = async () => {
  await waitForAuthorization();
  await balance2();
};

subscribeToAuthorizationAndGetBalance();
...
const subscription2 = observable.subscribe((message) => {
  // Exibe os dados recebidos no console para depuração
  console.log("vix50:sub2")+console.log(message)
  ...
  if (
    ...
    getTicksHistory();
    console.log("vix50:ressub2")+console.log(message.data.ohlc.epoch)

    console.log("vix50:start:"+startTime+"vix50:end:"+endTime)
...
const subscription3 = observable.subscribe((message) => {
  console.log("vix50:subs3")+ console.log(message)
  if (
    message &&
    message.name === 'message' &&
    message.data &&
    ...
    console.log("vix50:ressub3: ")+console.log(message.data.candles)
    open3 = message.data.candles[3].open;
    console.log("vix50:open3 "+open3)
    close3 = message.data.candles[3].close;
    console.log("vix50:close3 "+close3)
    high3 = message.data.candles[3].high;
    console.log("vix50:high3 "+high3)
    low3 = message.data.candles[3].low;
    console.log("vix50:low3 "+low3)
    open2 = message.data.candles[2].open;
    console.log("vix50:open2 "+open2)
    close2 = message.data.candles[2].close;
    console.log("vix50:close2 "+close2)
    open1 = message.data.candles[1].open;
    console.log("vix50:open1 "+open1)
    close1 = message.data.candles[1].close;
    console.log("vix50:close1 "+close1)

    oc = open3-close3
    console.log("vix50:oc "+oc)
    ho = high3-open3
    console.log("vix50:ho "+ho)
    cl = close3-low3
    console.log("vix50:cl "+cl)
    hl = high3-low3
    console.log("vix50:hl "+hl)
    ochl = parseFloat(oc/hl.toFixed(2))
    console.log("vix50:ochl "+ochl)
    hohl = ho/hl
    console.log("vix50:hohl "+hohl)
    clhl = cl/hl
    console.log("vix50:clhl "+clhl)

    c1 = ochl >= 0.05
    console.log("vix50:c1 "+c1)
    c2 = ochl <= 0.3
    console.log("vix50:c2 "+c2)
    c3 = hohl <=0.05
    console.log("vix50:c3 "+c3)
    c4 = clhl >=0.5
    console.log("vix50:c4 "+c4)
    c5 = open1 > close1
    console.log("vix50:c5 "+c5)
    c6 = open2 > close2
    console.log("vix50:c6 "+c6)
    c7 = open3 > close3
    console.log("vix50:c7 "+c7)
    c8 = open1 > open2
    console.log("vix50:c8 "+c8)
    }
})
